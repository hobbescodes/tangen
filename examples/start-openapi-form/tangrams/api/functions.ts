...
import { buildPath, buildQuery, getClient } from "./client";
import {
...
export const listPets = async (params?: ListPetsParams) => {
  ...
  const { data, error } = await $fetch<ListPetsResponse>(url, {
    output: listPetsResponseSchema,
  });
...
export const getPet = async (params: GetPetParams) => {
  ...
  const { data, error } = await $fetch<GetPetResponse>(path, {
    output: getPetResponseSchema,
  });
  if (error) throw error;
  return data;
};
...
export const getUser = async (params: GetUserParams) => {
  ...
  const { data, error } = await $fetch<GetUserResponse>(path, {
