...
> **Note:** Scalar values must start with a valid prefix for your validator: `z.` for Zod, `v.` for Valibot, `type(` / `type.` for ArkType, or `Schema.` for Effect. See the [Configuration Reference](/docs#scalars-graphql-only) for examples with other validators.
